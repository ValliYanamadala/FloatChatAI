...
function FloatDetails() {
  const { id } = useParams();
  const activeId = id || floats[0]?.id || "2901234";

  const float = floats.find((item) => item.id === activeId) || floats[0];
  const profile = profiles[activeId] || profiles[float?.id] || Object.values(profiles)[0];
  const trajectory = [
            ...
            <div className="measurements-header">
              ...
              <button
                onClick={() => {
                  ...
                  downloadAnchor.setAttribute("download", `argo_${float.id}_profile_156.json`);
